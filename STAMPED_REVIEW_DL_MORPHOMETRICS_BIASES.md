...
> **Status:** baseline audit of the pre-conversion repository. Its evidence about the notebooks and scripts remains useful, but its proposed layout, tool choices, conversion phases, and unresolved-question list are superseded by the [conversion plan](conversion-plan.md), [architecture](architecture-and-provenance.md), [decision records](decisions/README.md), and [poster-derived result targets](result-targets.md).

The resolved direction is Pixi for multiple local and image-construction environments; direct Apptainer/Singularity construction of exact SIF images from tracked definitions and reviewed locks; DataLad Containers for SIF registration, retrieval, and direct scientific execution; and BABS for Slurm campaigns. Provenance-safe Pixi tasks may launch explicit `datalad containers-run` commands, while BABS lifecycle tasks use a tracked operations ledger; DataLad must never record only a Pixi task name. Candidate SIFs may be used for disposable debugging, but every retained or scientifically compared output is regenerated after the exact SIF is registered and durably retrievable. Dataset composition follows the pinned BIDS 1.11.1 Study dataset convention: an outer `DatasetType: study` dataset, raw BIDS data at `sourcedata/raw/`, and independently described derivative datasets. The initial pilot is all eligible scans in `ds007116`; ABCD selection and protected derivatives follow after access/DUC review. Historic NIH derivatives and the old `balanced_scans.csv` are not required to begin.
